...
Re-checked against the #91 Review-queue corpus (32 Tracks, `tools/recheck.py`): verified 10 → 12 ("My 1st Song", "A Long Walk"), no Track lost, and witness calls 15 → 5. The first corpus run lost "Mom Praying": Shazam put the feature credit in the title and AcoustID in the artist, so the claims read as a disagreement and the prompt told the witness AcoustID named something else (old prompt `consistent` 10/10, new 7/10). Agreement now compares the pair with the credit moved into the title, as written Tags already are (#56), and that Track takes the agreed path.
