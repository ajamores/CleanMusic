...
- Failure (2026-08-22): `extract_info(url, download=False, process=False)` returns `None` for an already-archived single video when `download_archive` is in the opts — yt-dlp applies the archive filter during extraction, not just at download. So the archive-skip detection silently never fired (printed a bare `0 verified, 0 queued`), and unit tests using a fake `YoutubeDL` didn't catch it because the fake returned an entry the real call suppresses. Shipped through review and a merge before a live run exposed it.
- Rule: to resolve a Source's entries for an archive-membership check, resolve **archive-free** (no `download_archive` in the opts, `extract_flat` so a playlist isn't fully re-extracted), then check membership with a *separate* `YoutubeDL` that has `download_archive` via `in_download_archive`. And confirm any yt-dlp integration assumption with a real call, not only a fake — the fake encodes your assumption, so it can't disprove it.
