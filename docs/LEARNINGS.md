...
- Rule: to tell an archived re-run from an empty Source, resolve the Source's ids **archive-free** (no `download_archive` in the opts, `extract_flat` so a playlist isn't fully re-extracted), then match by **video id** against the archive file yourself (`<extractor> <id>` lines — take the last field), NOT via `in_download_archive` (its extractor-keyed match breaks across `Youtube`/`YoutubeTab`). And confirm any yt-dlp integration assumption with a real call, not only a fake — the fake encodes your assumption, so it can't disprove it.
